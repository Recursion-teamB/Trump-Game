...
export default class BlackGameScene extends Phaser.Scene {
  ...
  private dealerCardImage: Phaser.GameObjects.Image | null = null;
  ...
  public flipDealerSecondCard(): void {
    this.cardManager.flipDealerSecondCard();
  }
  public setDealerCardImage(dealerCardImage: Phaser.GameObjects.Image) {
    this.dealerCardImage = dealerCardImage;
  }
  public getDealerCardImage() {
    return this.dealerCardImage;
  }
}
...
export class CardManager {
  ...
  private dealerSecondCard: Phaser.GameObjects.Image | null = null;
  ...
  public dealCardToPlayer(player: BlackJackPlayer, card: Card, positionX: number, positionY: number, flipCard: boolean, duration: number = 200): void {
    ...
    if (cardImage) {
      ...
      if (player === this.table.getHouse() && !flipCard) {
        this.scene.setDealerCardImage(cardImage);
      }
    }
  }
  ...
  public firstDealCardToAllPlayers(dealerPosition: { x: number; y: number }, playerPositions: { [key: string]: { x: number; y: number } }) {
    ...
    for (let i = 0; i < 2; i++) {
      this.table.getPlayers().forEach((player, index) => {
        const card = this.table.getDeck().draw();
        const playerPosition = playerPositions[player.getName()];
        const dealDelay = (i * (this.table.getPlayers().length + 1) + index) * (dealDuration + delayBetweenCards);
  
        this.animateDealCard(card, player, playerPosition.x, playerPosition.y, dealDuration, dealDelay, i, index, true);
      });
  
      const card = this.table.getDeck().draw();
      const notFinal = i === 0;
      const dealDelay = (i * (this.table.getPlayers().length + 1) + this.table.getPlayers().length) * (dealDuration + delayBetweenCards);

      this.animateDealCard(card, this.table.getHouse(), dealerPosition.x, dealerPosition.y, dealDuration, dealDelay, i, -1, notFinal);
      
    }
  }
  
  public animateDealCard(card: Card, player: BlackJackPlayer, x: number, y: number, duration: number, delay: number, iteration: number, index: number, faceUp: boolean) {
    this.scene.tweens.add({
      targets: card,
      x: (x - this.cardWidth / 2) + iteration * (this.cardWidth + 6),
      y: y - this.cardHeight / 2 - 2,
      duration: duration,
      delay: delay,
      onStart: () => {
        this.dealCardToPlayer(player, card, (x - this.cardWidth / 2) + iteration * (this.cardWidth + 6), y - this.cardHeight / 2 - 2, faceUp);
      },
      onComplete: () => {
        if (index >= 0) {
  ...
  public flipDealerSecondCard(): void {
    if (this.dealerSecondCard) {
      const card = this.dealerSecondCard.getData('card') as Card;
      this.flipOverCard(card, this.dealerSecondCard);
    }
  }
}
