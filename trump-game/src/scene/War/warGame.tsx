...
export class WarScene extends Phaser.Scene {
    ...
    private drawnCardImages: Phaser.GameObjects.Image[] = [];
    ...
    createPlayerCard(card: Card, x: number, y: number) {
      ...
      cardBackImage.on('pointerdown', async () => {
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        const judgement = winner === "draw" ? "draw" : `${winner} wins!`;
        await this.createEventDisplay(judgement);
        if(winner === "draw"){
          this.drawnCardImages.push(cardBackImage, cpuCardBackImage!);
        }
        else{
          this.slideCardToJail([cardBackImage, cpuCardBackImage!, ...this.drawnCardImages], winner);
          this.drawnCardImages = [];
        }
        // スコアを更新
        this.updateScoreArea();
        // ゲームオーバーチェック
